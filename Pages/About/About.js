...
import { AiFillUniverse, AiFillCodepenCircle, AiOutlineStar, AiOutlineFork } from "react-icons/ai";
...
const About = (props) => {
    ...
    const contactList = Object.freeze([
        ...
        { type: "Universe", title: "Universe", url: "https://www.Universe.com/codepugilist", icon: (<AiFillUniverse style={{ fontSize: "35px" }} />), id: "Universe" },
    ...
    useEffect(() => {
        ...
        setLoading(true);
        Api().get('https://api.github.com/repos/Mahmoud-farargy/Universe-clone').then(response => {
            if (_isMounted.current) {
                setLoading(false);
                const { stargazers_count = 100, forks_count = 100, html_url = "https://github.com/Mahmoud-farargy/Universe-clone" } = response.data;
                if (response.data) {
                    setGithubInfo({
                        ...githubInfo,
